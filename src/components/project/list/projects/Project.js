            ...
            <TextWrapper>
              ...
              <Tags>
                {project.tags.map((tag, index) => {
                  if (index < 3) {
                    return <Tag key={tag.tag} style={{ color: tag.color }}>{`#${tag.tag}`}</Tag>;
                  } else if (index === 3) {
                    return <TagsText key={tag.tag}>{` + ${project.tags.length - 3}`}</TagsText>;
                  } else return null;
                })}
              </Tags>
            </TextWrapper>
            <ProjectClass projectClass={project.moreInfo.projectClass} />
            <DevelopState developState={project.moreInfo.developState} />
          </ProjectWrapper>
        ))}
    </Wrapper>
  );
...
const ProjectClass = ({ projectClass }) => {
  ...
  return (
    <ProjectClassWrapper style={{ backgroundColor: ClassList[projectClass].color }}>
